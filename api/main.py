...
# Duration of a session until finished by no action. Expressed in seconds.
app.config['SESSION_TIME_IN_SECONDS'] = 30 * 60
# Minimum time until the ending of a session, less than that requires a new token. In seconds.
app.config['SESSION_MIN_TIME_IN_SECONDS'] = 5 * 60
...
db, first_instance = DatabaseInstance.wait_for_connection_and_create_instance(wait_time=5, attempts=-1)
